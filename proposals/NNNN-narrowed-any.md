...
The right shape is therefore an *anonymous* closed-class-of-conformers type — the conformer set is named (each leaf is a real, nominal Swift type), but the type that bundles them is not. Swift already has all the infrastructure for this: `any P` is an existential, `any` carries a witness table, `is` / `as` open the box, `switch case let _ as T:` is the dispatch pattern. All of that machinery applies, unchanged, to a closed-class-of-conformers existential — the only addition is that **the compiler computes the members visible on the box from the join of the listed alternatives instead of from a single protocol's requirements**. The visible-interface computation specifically borrows the [§3.6 *join*][scala-union] concept from Scala 3, which is what determines members visible on the box (the most-derived class or protocol the leaves all conform to). The proposal does *not* port Scala 3 union types' broader semantics — commutativity, unboxing, untagging — which it explicitly rejects in favour of [Spelling is identity](#spelling-is-identity); see § [Alternatives considered](#alternatives-considered) for the contrast. The "join concept" piece is what Ceylon, Scala 3, Crystal, Python (PEP 604), and TypeScript have all shipped without language-design controversy; the *spelling-as-identity* + *closed nominal conformer set* parts are this proposal's deliberate choices to make Codable / witness / mangling tractable in Swift's nominally-typed setting.
