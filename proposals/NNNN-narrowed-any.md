...
Prototype on a fork of `swiftlang/swift` at [miku1958/swift][fork-swift], branch [`narrowed-any/phase1-poc`][fork-branch]. 13 lit tests at [`swift/test/NarrowedAny/`][fork-tests], 13/13 pass in ~17 seconds (12 runtime-positive files exercising end-to-end behaviour — 11 single-file `%target-run-simple-swift` plus 1 split-file `cross_module.swift` covering driver-build + `%target-run` — plus 1 `%target-typecheck-verify-swift` file locking in the negative-path Sema diagnostics). The bullets below list every user-visible capability the prototype exercises today; the gap list afterward names what still has to land before the proposal is reviewable as a v1.
