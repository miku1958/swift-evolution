...
- <a id="strict-invariant-silent-passes"></a>**Strict-invariant rule has silent passes in the constraint solver**: § [Subtyping lattice](#subtyping-lattice) commits container types to invariant in their element type and function types to strict invariant in narrowed-`Any` positions, but the prototype's constraint solver currently lets four conversions through silently — each via a stdlib `collection_upcast` or `function_conversion` path that doesn't yet consult `NarrowedAnyType`. Each needs a Sema short-circuit plus a matching verify-mode annotation:
  - **Container leaf-injection at direct binding** — `let _: [Int | String] = xs` for `xs: [Int]` is silent today. The same conversion at the *extension-dispatch* boundary already errors with the auto-fix-it documented above (`diagnostics.swift §8b`); the direct-binding form needs the same diagnostic and ` as [Int | String]` fix-it (O(N) per-element wrap, same SIL path).
  - **Container narrowing to `Any`** — `let _: [Any] = zs` for `zs: [Int | String]` is silent today via `collection_upcast`'s element-wise erasure path. Should require an explicit `as [Any]` (free SIL relabel — both sides hold the Any-singleton element layout, so no per-element work is needed).
  - **Set element widening** — `let _: Set<Int | String> = s` for `s: Set<Int>` is silent today, same root cause as the Array widening case (`collection_upcast` plus the auto-synthesised `Hashable` conformance on `Int | String`). Same policy: explicit `as Set<Int | String>` with the per-element wrap fix-it.
  - **Function-type parameter narrowing** — `let _: (Int) -> Void = f` for `f: (Int | String) -> Void` is silent today via `function_conversion`'s parameter contravariance. Should reject; the relaxation (parameter-contravariant subtyping for narrowed-`Any` positions) is deferred to § [Variance at the protocol-witness boundary](#variance-at-the-protocol-witness-boundary). The reverse direction `(Int) -> Void` → `(Int | String) -> Void` already errors (locked in `diagnostics.swift §19`); the asymmetry is the gap.

  The fix lives in `lib/Sema/CSSimplify.cpp` — the collection-upcast and function-conversion paths need to short-circuit when either side carries a `NarrowedAnyType`. The negative-path infrastructure (verify-mode annotations, fix-it generation, runtime SIL relabel) is the same as `diagnostics.swift §13`/`§17`/`§18`/`§19`. Tracked as a v1-blocking follow-up; the prototype's existing positive runtime tests (`phase2f_runtime.swift §1`/`§2`) cover the explicit-cast path so the ABI shape is settled — only the diagnostic surface needs to land.
