...
- <a id="strict-invariant-silent-passes"></a>**Strict-invariant rule's container axis now errors; function-type axis still silent**: § [Subtyping lattice](#subtyping-lattice) commits container types to invariant in their element type and function types to strict invariant in narrowed-`Any` positions. Of the four implicit-conversion paths that originally let invariance violations through silently, the three on the container axis now error with the standard generic-argument-mismatch wording (`lib/Sema/CSSimplify.cpp`'s `ArrayUpcast` / `DictionaryUpcast` / `SetUpcast` cases force `Bind` on the element when either side carries a `NarrowedAnyType`, so the leaf-injection / open-erasure subtype routes no longer fire); the function-type axis is the remaining gap. Locked by `diagnostics.swift §17`/`§18`/`§19`/`§20` for the now-erroring shapes.
  ...
  - **Function-type parameter narrowing** — `let _: (Int) -> Void = f` for `f: (Int | String) -> Void` is still silent today via `function_conversion`'s parameter contravariance. Should reject; the relaxation (parameter-contravariant subtyping for narrowed-`Any` positions) is deferred to § [Variance at the protocol-witness boundary](#variance-at-the-protocol-witness-boundary). The reverse direction `(Int) -> Void` → `(Int | String) -> Void` already errors (locked in `diagnostics.swift §19`); the asymmetry is the gap. The fix lives at the function-conversion subtype check in `lib/Sema/CSSimplify.cpp::matchFunctionTypes`, parallel to the container axis above. Tracked as a v1-blocking follow-up.

  Explicit `as` coercion bypasses the `Bind` tightening — the constraint solver detects the `CoerceExpr` anchor before forcing invariance on the element types — so cross-spelling reshape (`zs as [String | Int]`) and explicit widening / narrowing (`xs as [Int | String]`, `zs as [Any]`) all continue to work via the SIL paths exercised by `phase2_edge.swift §8` and `phase2f_runtime.swift §1`/`§2`.
