...
The headline use case is **typed throws across heterogeneous error domains** — the sub-feature [SE-0413] explicitly left for follow-up work. Today's workarounds (wrapper enums, untyped `throws`, custom `Either<A, B>` library types) scale quadratically (`O(N×M)` in API surface × error sources per API) and do not compose across library boundaries. The narrowed-`Any` form replaces all of them with one syntactic construct that composes linearly (`O(N+M)`) and unblocks the rest of the typed-throws story at the same time: exhaustive cross-domain `catch`, generic propagation through `rethrows`-style code, untagged `Codable` for sloppy JSON, and `where T == A | B` generic constraints.
...
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **narrowed `Any`** *(current)* | "the type `A \| B` is an `Any` narrowed to a closed set of dynamic types"                                                                    | Connects directly to a concept Swift users already know (`Any`). Emphasises the *value-layout* truth: it really is an existential `Any` with a closed conformer set. Foregrounds "this is not a union" upfront. | Less precise once you reach the**constraint** position: `where T == A \| B` binds T to the alternation rather than narrowing one specific type. Two-word term, requires the reader to internalise that `Any` here is the noun. |
| **Closed Type Set**                | "`A \| B` denotes the closed type set `{A, B}`; a value's dynamic type belongs to the set; `where T == A \| B` says T is bound to the closed set itself (with future set-membership form `where T: A \| B` saying T is a *member* of the set)" | Reads naturally at both value*and* constraint level. Aligns with the closed-set semantics; the future set-membership relaxation slots in naturally. Honest about the closed-class-of-conformers nature. Avoids "union" entirely.                                                       | Three words; risks visual collision with the standard library's `Set<T>` type. Newer term — no existing Swift mental hook.                                                                       |
...
// consider using a generic constraint instead". v1 keeps this strict;
// the conformance is already synthesised, only the value-member-lookup
// hook is deferred — see § [Future directions § Direct member access on
// a narrowed-`Any` value](#direct-member-access-on-a-narrowed-any-value).
// v.description                     // deferred: route via describe(v) / "\(v)" / (v as any P) today
...
**v1 prototype status.** The per-witness conformance synthesis itself **is wired** for the standard library's protocols with method requirements — `Equatable`, `Hashable`, `Comparable`, `CustomStringConvertible`, `Encodable`, `Decodable` — via the builtin `NarrowedAnyDispatch` conformance kind in [`lib/AST/ConformanceLookup.cpp`][fork-conformance-lookup] plus per-leaf SIL dispatch helpers in [`lib/SILGen/SILGenType.cpp`][fork-silgen-type]. So `describe(v)`, string interpolation `"\(v)"`, `Set<Int | String>`, `JSONEncoder().encode(v)`, etc. all work in v1 (validated by [`phase3f_*.swift`][fork-tests] lit tests). What v1 does **not** ship is the *direct* value-member-access form (`v.description`); the conformance is already synthesised but the Sema member-lookup path on a narrowed-`Any` receiver does not yet consult it. Reaching `any P` parity on that one path is deferred to a focused follow-up — see [Future directions § Direct member access on a narrowed-`Any` value](#direct-member-access-on-a-narrowed-any-value) — because the lookup-site change plausibly touches every overload-ranking spot that already special-cases `any P` and warrants a separate audit. Until then the conformance is reachable through the generic-dispatch / interpolation / any-cast paths shown above.
...
func f<T>(_: T) where T == Int | String
extension Array where Element == Int | String { ... }
...
* Extension dispatch on a generic with a narrowed-`Any` element constraint applies to receivers whose generic argument matches the constraint exactly: `extension Array where Element == Int | String { … }` matches `[Int | String]`. Leaf-typed receivers (`[Int]`, `[String]`) and cross-spelling receivers (`[String | Int]`) need an **explicit cast** in v1 — the compiler emits a fix-it that inserts `(receiver as [A | B])` for the leaf case and `(receiver as [A | B])` for the cross-spelling case (one keystroke each). The two axes have separate deferred follow-ups, because relaxing each one is a different mechanism: implicit leaf-injection is the [Per-element leaf injection at the extension boundary](#per-element-leaf-injection-at-the-extension-boundary) future direction (per-element layout differs between `[Int]` and `[Int | String]`, requiring runtime element-wrap or specialisation work), and implicit cross-spelling is gated on the [Order-insensitive marker in `where` clauses](#order-insensitive-marker-in-where-clauses) future direction (the user opts a single binding into matching against any spelling whose sorted leaves agree, via a where-clause keyword). The v1 ergonomic story is "fix-it makes the cast trivial". § [Containers and extensions](#containers-and-extensions) covers all three axes.
...
| **Per-witness dispatch** | `Hashable`, `Equatable`, `Comparable`, `CustomStringConvertible`, `Encodable`, `Decodable`                | **Working in the prototype.** Implemented via a dedicated `BuiltinConformance` kind `NarrowedAnyDispatch` ([`lib/AST/ConformanceLookup.cpp::lookupExistentialConformance`][fork-conformance-lookup]) plus six per-protocol SIL dispatch helpers in [`lib/SILGen/SILGenType.cpp`][fork-silgen-type] that emit witness thunks opening the existential and forwarding to each leaf's own conformance. So `JSONEncoder().encode(v)`, `Set<Int \| String>`, `<` over `Int \| Double`, `\(v)` interpolation, etc. all work today (validated by [`phase3f_codable_*.swift`][fork-tests], [`phase3f_equatable.swift`][test-phase3f-equatable], plus the Codable round-trip in [`codable_untagged.swift`][test-codable-untagged]). |
...
**Direct value-member access is deferred.** `v.description` for `v: Int | String` is currently rejected even though the `CustomStringConvertible` conformance is fully synthesised — the conformance is reachable via generic-position dispatch (`describe(v)`), string interpolation (`"\(v)"`), and any-cast (`(v as any CustomStringConvertible).description`), but not directly. Wiring the Sema value-member-lookup path on a narrowed-`Any` receiver to consult the synthesised builtin conformance is its own focused follow-up; v1 keeps the strict-reject diagnostic and routes users through the workarounds above. See [Future directions § Direct member access on a narrowed-`Any` value](#direct-member-access-on-a-narrowed-any-value).
...
<a id="generics-where-t-a--b"></a>
### Generics: `where T == A | B`

A constraint `where T == A | B` is the **same-type degraded form**: `T` is bound to the alternation type `A | B` itself. Spelling-as-identity applies as everywhere else — `where T == A | B` and `where T == B | A` are *different* constraints (their bindings of `T` have different mangled names and witness identities, per [Spelling is identity](#spelling-is-identity)). A caller holding a value spelled `B | A` reshapes via an explicit `as` cast at the call site (free SIL relabel; the cast surfaces the spelling change). The body of the function always sees `T` as the alternation regardless of which leaf the caller passed.

```swift
func process<T>(_ error: T) where T == NetworkError | DecodingError {
    switch error {
    case let e as NetworkError:  ...   // exhaustive: the constraint guarantees T's leaves are these two
    case let e as DecodingError: ...
    }
}

process(NetworkError.timeout)         // leaf passed; injects into NetworkError | DecodingError
process(DecodingError.malformed)      // leaf passed; injects into NetworkError | DecodingError
let e1: NetworkError | DecodingError = ...
process(e1)                           // alternation passed; same spelling, trivial identity
let e2: DecodingError | NetworkError = ...
process(e2)                           // ERROR: same leaf set but different spelling. Spelling-as-identity
                                      // applies: `T == NetworkError | DecodingError` and the value's type
                                      // `DecodingError | NetworkError` are different (different mangled name,
                                      // different witness identity). Reshape is a runtime-free relabel but
                                      // must be explicit so the user sees the spelling change.
                                      // Fix-it: `process(e2 as NetworkError | DecodingError)`.
```

The body type-checks against the *join* of the alternation's leaves — the body of `process` may use any member that every leaf provides, but cannot use leaf-only members without first narrowing with `as?`. This matches what the body of a function taking `A | B` directly already sees.

<a id="colon-form-reserved"></a>
**The `where T: A | B` colon form is rejected by the parser in v1**, with a diagnostic pointing the user at the `where T == A | B` form. The colon-form syntax slot is **reserved for the [True set-membership](#true-set-membership-for-where-t-a--b) future direction**, where it gains a different semantics: `T` becomes a *member* of the closed leaf set (the body can specialise to a single leaf when the substitution actually is one), and matching becomes order-free at the call site. Because v1 commits cleanly to `==`, code written today will not silently change semantics if and when the colon form lands later — the future direction is purely additive.
...
**Throws position.** `throws(A | B)` is a *type position*: the spelling is part of the function signature's identity. A protocol method declared `throws(NetworkError | DecodingError)` and an implementation written `throws(DecodingError | NetworkError)` are two *different* signatures; the compiler reports "does not conform to protocol" with a fix-it that reorders the implementation. The same Spelling-as-identity rule applies at constraint position too — `where T == A | B` and `where T == B | A` are different constraints, and a caller passing a value spelled `B | A` reshapes via an explicit `as` at the call site (free SIL relabel). § [Issue 6](#issue-6-generic-constraints-where-t-a--b) covers the constraint-position rule in full.
...
**v1 conservatism + two future relaxations.** v1 commits to [Spelling is identity](#spelling-is-identity) everywhere a narrowed-`Any` is bound to a name — including the constraint position. `where T == A | B` and `where T == B | A` are *different* constraints (the bindings have different mangled names and witness identities), and a caller passing a value spelled `B | A` into a function declared with `T == A | B` reshapes through an explicit `as` cast (with a fix-it surfacing the cross-spelling change). What v1 *does not* give you is (a) the colon form `where T: A | B` — rejected by the parser, with the syntax slot reserved for full set-membership specialisation where the body sees `T` as a *single bound leaf* — and (b) cross-spelling matching at extension-`where Element == ...` clauses, where v1 honours [Spelling is identity](#spelling-is-identity). Both relaxations are sketched as Future directions: § [True set-membership for `where T: A | B`](#true-set-membership-for-where-t-a--b) addresses the binding axis (the colon form ships there with disjunctive requirements at the constraint solver level, including the natural order-free leaf-set match at constraint position), and § [Order-insensitive marker in `where` clauses](#order-insensitive-marker-in-where-clauses) addresses the spelling axis at extension-clause level via an explicit opt-in marker.
...
* `where T == A | B` and `throws(A | B)` parse `A | B` as a type term in both positions, but the role differs: the constraint-position form binds `T` to the alternation type (same-type degraded form; see [Generics](#generics-where-t-a--b)), while the type-position form (function signature, including `throws(...)`) makes the spelling part of the signature's identity. The colon form `where T: A | B` is **rejected by the parser in v1** — its syntax slot is reserved for the [True set-membership](#true-set-membership-for-where-t-a--b) future direction.
...
| **Generic constraint** `where T == A \| B` (same-type to the alternation) | ABI break — adding a leaf changes the bound type's mangled name. Source: callers spelled with the old set must add the new leaf to keep matching.       | ABI break — same reason. Source-break — uses that relied on the removed leaf no longer satisfy the constraint.               |
...
The proposal commits to **#1 as a focused follow-up** (most contained scope, biggest immediate UX win for Codable-heavy users) and lists #2–#4 as longer-term avenues. v1 ships the try-each behaviour with the breakpoint UX as-is, matching every other `try?`-based fallback chain in Swift today.
...
<a id="issue-6-generic-constraints-where-t-a--b"></a>
### Issue 6: Generic constraints `where T == A | B`

**Question.** What does `where T == A | B` mean? Does order matter? Can you write multi-clause `where T == A | B, T == C | D`? Can `&` appear anywhere? What about the colon form `where T: A | B` — does that mean something different?

**Proposed resolution.** *Same-type degraded form.* `where T == A | B` binds `T` to the alternation type `A | B` itself; the body sees `T` as the alternation (not as a single leaf when the caller passed one). The substitution check is **spelling-aware**: `where T == A | B` and `where T == B | A` are different constraints because they produce bindings of `T` with different mangled names and witness identities — [Spelling is identity](#spelling-is-identity) extends to constraint position. A caller that has a value spelled `B | A` and wants to call a function declared with `T == A | B` reshapes via an explicit `as` cast at the call site (free SIL relabel; the cast surfaces the spelling change).

**The colon form `where T: A | B` is parser-rejected in v1.** v1 commits cleanly to the same-type form `where T == A | B`. The colon-form syntax slot is **reserved** for the [True set-membership](#true-set-membership-for-where-t-a--b) future direction, where it gains a different (strictly more permissive) semantics: `T` becomes a *member* of the closed leaf set, the body can specialise to a single leaf when the substitution actually is one, and matching becomes order-free at the call site (where `where T: A | B` and `where T: B | A` accept the same set of substitutions and pick the right binding from the call-site value). True set-membership requires disjunctive requirements at the constraint-solver level; that is its own substantial project. By keeping v1's surface to `==` only, code written today does not silently change semantics if and when colon form lands later — the future direction is purely additive.

**Single narrowed-`Any` constraint per type parameter.** Multi-clause (`where T == A | B, T == C | D`) is rejected — there is no obvious correct meaning (intersection? union of unions?) and the diagnostics for the two interpretations are confusing. The same ban applies trivially to combining `T == A | B` with any other constraint that would itself rebind `T` (Swift's existing rule: "no type for 'T' can satisfy both 'T == X' and 'T : SomeProto'" already rejects mixing same-type with conformance constraints, so writing `where T == A | B, T: Error` falls under that existing diagnostic — `Error` conformance for the alternation, when both leaves conform, is auto-synthesised through self-conforming protocol dispatch and does not need to be re-asserted as a separate clause). § [Issue 9](#issue-9-interaction-with--protocol-composition--superclass) covers the related ban on `&`-with-narrowed-`Any`.
...
Prototype on a fork of `swiftlang/swift` at [miku1958/swift][fork-swift], branch [`narrowed-any/phase1-poc`][fork-branch]. Lit tests at [`swift/test/NarrowedAny/`][fork-tests] cover runtime-positive behaviour end-to-end (single-file `%target-run-simple-swift` plus a split-file [`cross_module.swift`][test-cross-module] covering driver-build + `%target-run`) and a `%target-typecheck-verify-swift` file locking in the negative-path Sema diagnostics. The bullets below list every user-visible capability the prototype exercises today; the gap list afterward names what still has to land before the proposal is reviewable as a v1.
...
- **Per-witness dispatch** for stdlib protocols with method requirements — `Equatable`, `Hashable`, `Comparable`, `CustomStringConvertible`, `Encodable`, `Decodable` — implemented via the `BuiltinConformance(NarrowedAnyDispatch)` kind in [`lib/AST/ConformanceLookup.cpp`][fork-conformance-lookup] plus six per-protocol SIL dispatch helpers in [`lib/SILGen/SILGenType.cpp`][fork-silgen-type]. `JSONEncoder().encode(v)`, `Set<Int | String>`, `Comparable.<` over `Int | Double`, `\(v)` interpolation through the `CustomStringConvertible` witness — all work today; validated by [`phase3f_codable_*.swift`][fork-tests] and [`phase3f_equatable.swift`][test-phase3f-equatable].
...
- Generic `where T == A | B` (same-type degraded form; the colon form `where T: A | B` is parser-rejected with a diagnostic pointing at `==`, with the syntax slot reserved for [Future directions § True set-membership](#true-set-membership-for-where-t-a--b)).
...
- **Per-element leaf injection fix-it at the extension boundary**: when a leaf-typed receiver (`xs: [Int]`) reaches an extension on a narrowed-`Any` element (`extension Array where Element == Int | String`), v1 emits the existing same-type-requirement error with a fix-it that inserts `(receiver as [Int | String])` before the call. The fix-it makes the cast one keystroke; users see "error → click → fixed". Wired in [`lib/Sema/CSDiagnostics.cpp`][fork-csdiagnostics]'s `RequirementFailure::diagnoseAsError` — fires when the requirement is `SameType`, the rhs is `NarrowedAnyType`, and the lhs is a leaf of the rhs (recursively, including nested narrowed-`Any`); locks in by [`diagnostics.swift`][test-diagnostics] §8b verify-mode annotations. The *implicit* form (no cast at all) is deferred to a follow-up — see [Future directions § Per-element leaf injection at the extension boundary](#per-element-leaf-injection-at-the-extension-boundary). Explicit-cast (`(xs as [Int | String]).method()`) is verified working end-to-end (test bed [`phase2_edge.swift`][test-phase2-edge] §8); the SIL path for the cast itself is mature.

  **Cross-spelling diagnostic-quality gap** (`zs: [Int | String]` reaching `extension Array where Element == String | Int`): the diagnostic for that axis surfaces through a *different* code path — per-alternative type comparison emitting "any Int" vs. "any String" — which doesn't carry narrowed-`Any` context, so the fix-it above doesn't activate. The cross-spelling cast itself works end-to-end via explicit `(zs as [String | Int]).method()` (runtime-free relabel, validated in [`phase2_edge.swift`][test-phase2-edge] §8); only the diagnostic quality is below the leaf-injection bar. A separate Sema follow-up reshapes the diagnostic emission to recognise the narrowed-`Any` parent types and attach an analogous fix-it. Marked as a known v1 limitation in [`diagnostics.swift`][test-diagnostics] §8a; not blocked on v1 review.
- <a id="strict-invariant-silent-passes"></a>**Strict-invariant rule's container axis now errors; function-type axis still silent**: § [Subtyping lattice](#subtyping-lattice) commits container types to invariant in their element type, and § [Variance at the protocol-witness boundary](#variance-at-the-protocol-witness-boundary) keeps function types strict-invariant in narrowed-`Any` positions (with the parameter-contravariant / return-covariant relaxation explicitly deferred). Of the four implicit-conversion paths that originally let invariance violations through silently, the three on the container axis now error with the standard generic-argument-mismatch wording ([`lib/Sema/CSSimplify.cpp`][fork-cssimplify]'s `ArrayUpcast` / `DictionaryUpcast` / `SetUpcast` cases force `Bind` on the element when either side carries a `NarrowedAnyType`, so the leaf-injection / open-erasure subtype routes no longer fire); the function-type axis is the remaining gap. Locked by [`diagnostics.swift`][test-diagnostics] §17/§18/§19/§20 for the now-erroring shapes.
  - **Container leaf-injection at direct binding** ✅ — `let _: [Int | String] = xs` for `xs: [Int]` now errors. Recovery is an explicit ` as [Int | String]` (per-element wrap because the source `Array<Int>` element stride 8 bytes differs from the destination `Array<Int | String>` element stride for the Any-singleton layout); positive runtime bed is [`phase2f_runtime.swift`][test-phase2f-runtime] §1. The same conversion at the extension-dispatch boundary continues to use the auto-fix-it from [`diagnostics.swift`][test-diagnostics] §8b.
  - **Container narrowing to `Any`** ✅ — `let _: [Any] = zs` for `zs: [Int | String]` now errors. Recovery is an explicit ` as [Any]` (free SIL relabel — both sides hold Any-singleton element layout, so no per-element work).
  - **Set element widening** ✅ — `let _: Set<Int | String> = s` for `s: Set<Int>` now errors via the same path as the Array widening case (the auto-synthesised `Hashable` conformance on `Int | String` keeps the Set side well-formed; the strict-invariant rejection is what was missing).
  - **Function-type parameter narrowing** — `let _: (Int) -> Void = f` for `f: (Int | String) -> Void` is still silent today via `function_conversion`'s parameter contravariance. Should reject; the relaxation (parameter-contravariant subtyping for narrowed-`Any` positions) is deferred to § [Variance at the protocol-witness boundary](#variance-at-the-protocol-witness-boundary). The reverse direction `(Int) -> Void` → `(Int | String) -> Void` already errors (locked in [`diagnostics.swift`][test-diagnostics] §19); the asymmetry is the gap. The fix lives at the function-conversion subtype check in [`lib/Sema/CSSimplify.cpp::matchFunctionTypes`][fork-cssimplify], parallel to the container axis above. Tracked as a v1-blocking follow-up.

  Explicit `as` coercion bypasses the `Bind` tightening — the constraint solver detects the `CoerceExpr` anchor before forcing invariance on the element types — so cross-spelling reshape (`zs as [String | Int]`) and explicit widening / narrowing (`xs as [Int | String]`, `zs as [Any]`) all continue to work via the SIL paths exercised by [`phase2_edge.swift`][test-phase2-edge] §8 and [`phase2f_runtime.swift`][test-phase2f-runtime] §1/§2.
...
Until this lands, user-written protocols on narrowed-`Any` route through the v1 fallback ("doesn't conform" diagnostic at compile time), and users can either constrain their generic via `where T == A | B` (which works in v1 by binding `T` to the alternation type and routing protocol calls through the alternation's auto-synthesised conformance) or wait for the generalisation.
...
<a id="direct-member-access-on-a-narrowed-any-value"></a>
### Direct member access on a narrowed-`Any` value

In v1, `let v: Int | String = "hi"; v.description` is rejected at the type-check site even though the `CustomStringConvertible` conformance for `Int | String` is fully synthesised through the per-witness dispatch route in [§ Conformance synthesis (v1 scope)](#conformance-synthesis-v1-scope). The conformance is reachable today through:

- Generic-position dispatch: `func describe<T: CustomStringConvertible>(_ x: T)`, `describe(v)` — works.
- String interpolation: `"\(v)"` — works.
- Explicit any-cast: `(v as any CustomStringConvertible).description` — works.

Only the *direct* member-access form on the narrowed-`Any`-typed value rejects. This is a Sema-side gap: the Sema member-lookup path for narrowed-`Any` does not yet consult the synthesised builtin conformance the way the lookup path for `any P` already does.

A follow-up wires that lookup so a narrowed-`Any` value supports direct member access for every join-visible member, matching `any P` ergonomics. The conformance machinery is already in place — [`lib/AST/ConformanceLookup.cpp::lookupConformance`][fork-conformance-lookup] returns the `BuiltinConformance(NarrowedAnyDispatch)` for any of the v1 hardcoded protocols, and the witness thunk in [`lib/SILGen/SILGenType.cpp`][fork-silgen-type] handles the dispatch — so the work is exercising that conformance from the member-lookup site ([`lib/Sema/CSSimplify.cpp`][fork-cssimplify]'s value-member-lookup path) when the receiver type is a `NarrowedAnyType`. The shape mirrors how Sema reaches the conformance through the join in generic-position dispatch already.

Two reasons this is deferred rather than done in v1:

1. **The workarounds are explicit and discoverable.** Generic-position dispatch and string interpolation both work; for the rare case where neither fits, `(v as any CustomStringConvertible).description` is a one-line escape hatch. v1 does not block any expressible code on this gap.
2. **Sema-side risk.** The "machinery is already wired" framing oversimplifies — connecting the value-member-lookup path to `BuiltinConformance(NarrowedAnyDispatch)` plausibly touches every overload-ranking site that already special-cases `any P`, and shipping it on a tight v1 timeline without an exhaustive audit risks regressing those paths. A focused follow-up pays the audit cost cleanly.

When the follow-up lands, member access on a narrowed-`Any` value is sourcekit-completable like member access on `any P`, exhaustively for the join-visible member set. Until then, the workaround is to reach the witness through the generic / interpolation / any-cast paths above.
...
This future direction covers the **leaf-injection axis only** — a single-leaf-typed receiver (`xs: [Int]`) reaching an extension declared on a wider narrowed-`Any` element (`extension Array where Element == Int | String`). The *cross-spelling* axis at extension dispatch (`zs: [String | Int]` reaching the same extension; same leaf set, different spelling) is a separate concern with its own future direction — see [Order-insensitive marker in `where` clauses](#order-insensitive-marker-in-where-clauses) — because relaxing it requires a user-facing opt-in keyword on the where clause, not a Sema/SILGen-only change.

v1 ships leaf-injection extension dispatch as **explicit-cast-with-fix-it**: `xs: [Int]` reaching `extension Array where Element == Int | String` is rejected with a fix-it that inserts `(receiver as [Int | String])`. The cast walks the array element-wise, wrapping each leaf as an `Any`-singleton existential, because `[Int]` (8-byte stride) and `[Int | String]` (32-byte stride: 24-byte buffer + 8-byte metadata) have different per-element layouts — the conversion cannot be a runtime-free relabel.
...
Cross-spelling at extension dispatch sits outside this future direction by construction: the runtime cost is already zero (free SIL relabel), so the axis doesn't benefit from specialisation; what it needs is a way for the user to declare "I don't care which spelling reaches this extension". That's the [Order-insensitive marker in `where` clauses](#order-insensitive-marker-in-where-clauses) future direction.

### Order-insensitive marker in `where` clauses

Spelling-is-identity makes `[Int | String]` and `[String | Int]` distinct types, so generic code parameterised on `[T]` matches one shape only and every other shape needs an explicit cross-shape `as` at the call site (and at extension dispatch — `zs: [Int | String]` reaching `extension Array where Element == String | Int` is a v1 explicit-cast situation just like `xs: [Int]` reaching the same extension is, although the runtime is free for cross-spelling and O(N) for leaf-injection). A future surface could introduce a where-clause marker — strawman `where T == Int | String unordered` or `where T ~= Int | String` — that opts a single generic binding into matching against any spelling whose sorted leaves agree, so inside such a generic both `[Int | String]` and `[String | Int]` satisfy `[T]` directly. **This is the gate for implicit cross-spelling at extension dispatch**: an extension declared `extension Array where Element ~= Int | String { … }` accepts `[Int | String]` *and* `[String | Int]` without a cast, because the user has explicitly opted that binding into spelling-insensitivity. Cross-spelling at extension dispatch stays explicit by default; the marker is the user-visible affordance for "this extension doesn't care about spelling".

The constraint-solver hook parasitises the [two-layer cache](#compile-time-check): comparing the binding's interned sorted-leaves identity instead of its canonical-type pointer is `O(1)` after first encounter, no new caches required. Spelling-is-identity stays the global default; this is opt-in for code that genuinely doesn't care (set-algebra over leaves, JSON/Codable wrappers that round-trip through a sorted-canonical wire format, the IDE-side cross-shape completion below sharing the same index). Two open design questions: marker spelling needs evolution review, and protocol-witness dispatch needs an explicit decision on whether marker-bound dispatch follows the caller's spelling (preserves the dispatch-level "spelling is identity" guarantee that v1's [conformance-synthesis escape hatch](#conformance-synthesis-v1-scope) inherits) or normalises to sorted-canonical form (cheaper but loosens that guarantee — the same loosening v1 was framed to avoid).
...
Today a `switch` over a narrowed-`Any` lowers to a chain of `checked_cast_addr_br` calls into `swift_dynamicCast`. Because the alternative list is closed and known at compile time, the chain can be sunk into a `load_metadata` plus a metadata-pointer `switch_value` table — the same shape `enum` dispatch uses. Likewise `v as? Int` against `v: Int | String` can be reduced to a single metadata compare. These are local SIL transforms, not ABI changes; they sit in [`lib/SILOptimizer`][fork-siloptimizer] and apply on top of whatever route the conformance-synthesis design eventually lands on.
...
[fork-conformance-lookup]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/lib/AST/ConformanceLookup.cpp
[fork-csdiagnostics]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/lib/Sema/CSDiagnostics.cpp
[fork-cssimplify]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/lib/Sema/CSSimplify.cpp
[fork-silgen-type]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/lib/SILGen/SILGenType.cpp
[fork-siloptimizer]: https://github.com/miku1958/swift/tree/narrowed-any/phase1-poc/lib/SILOptimizer
...
[test-codable-untagged]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/codable_untagged.swift
[test-cross-module]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/cross_module.swift
[test-diagnostics]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/diagnostics.swift
[test-phase2-edge]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/phase2_edge.swift
[test-phase2f-runtime]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/phase2f_runtime.swift
[test-phase3f-equatable]: https://github.com/miku1958/swift/blob/narrowed-any/phase1-poc/test/NarrowedAny/phase3f_equatable.swift
